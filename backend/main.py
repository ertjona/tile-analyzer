...
# --- Caching (Restored Missing Functions) ---
_distribution_cache = {}
...
def get_summary_stats():
    conn = get_db_connection()
    total_files = conn.execute("SELECT COUNT(*) FROM SourceFiles").fetchone()[0]
    total_tiles = conn.execute("SELECT COUNT(*) FROM ImageTiles").fetchone()[0]
    conn.close()
    return {"total_source_files": total_files, "total_image_tiles": total_tiles}
